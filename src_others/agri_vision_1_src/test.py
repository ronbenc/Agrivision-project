...
def save_result(info, pred):
    ...
    for i in range(classes.shape[0]):
        ...
        # result_png = ToPILImage()(classes[i].float() / 40.)
        img_name = info[i]
        ...
        # img_name = cfg.TEST.result + img_name.replace('.jpg', '.png')
        img_name = cfg.DIR + cfg.TEST.result + img_name.replace('.jpg', '.png')
        result_png.save(img_name)
...
def main():
    ...
    cfg.DIR = os.path.join(cfg.DIR,
                           ...
                           datetime.now().strftime('_%Y_%m_%d_%a_%H_%M_%S_%f'))
    ...
    if args.local_rank == 0:
        ...
        os.makedirs(os.path.join(cfg.DIR, 'result'), exist_ok=True)
