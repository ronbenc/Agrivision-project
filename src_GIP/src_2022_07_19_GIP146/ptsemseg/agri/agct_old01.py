...
class agri_color_transform(nn.Module):
    ...
    def forward(self, img):
        ...
        denom = self.alpha[0][5] * img[:, 0, :, :] + self.alpha[0][8] * img[:, 3, :, :] + 1e-12
        C = nomin / denom
        ...
        denom = self.alpha[1][5] * img[:, 0, :, :] + self.alpha[1][8] * img[:, 3, :, :] + self.alpha[1][9] + 1e-12
        C = nomin / denom
        C = torch.nan_to_num(C, nan=0.0)
        CT[:, 1, :, :] = C
        ...
        denom = self.alpha[2][5] * img[:, 0, :, :] + self.alpha[2][7] * img[:, 2, :, :] + self.alpha[2][8] * img[:, 3, :, :] + self.alpha[2][9] + 1e-12
        C = nomin / denom
        C = torch.nan_to_num(C, nan=0.0)
        CT[:, 2, :, :] = C
        ...
        # denom = self.alpha[3][6] * img[:, 1, :, :] + 1e-12
        # C = nomin / denom
        # C = torch.nan_to_num(C, nan=0.0)
        # CT[:, 3, :, :] = C
        #
        # # ~ SIPI
        # nomin = self.alpha[4][2] * img[:, 2, :, :] + self.alpha[4][3] * img[:, 3, :, :]
        # denom = self.alpha[4][5] * img[:, 0, :, :] + self.alpha[4][8] * img[:, 3, :, :] + 1e-12
        # C = nomin / denom
        # C = torch.nan_to_num(C, nan=0.0)
        # CT[:, 4, :, :] = C
