...
def train(cfg):
    ...
    curr_acc_step = 1
    total_loss = 0.0
    agct_lr = np.array(cfg["agct"]["lr"]) / acc_step
    optimizer.defaults['lr'] =  optimizer.defaults['lr'] / acc_step
    ...
    while i <= cfg["training"]["train_iters"] and flag:
        ...
        for (images, labels) in trainloader:
            ...
            # total_loss = (total_loss + loss) / acc_step  # Normalize our loss (if averaged)
            # loss = (total_loss + loss) / curr_acc_step
            loss.backward()

            if (i + 1) % acc_step == 0:  # Wait for several backward steps
                ...
                if "agct" in cfg.keys():  # cfg["agct"] is not  None:
                    if cfg["agct"]["dynamic_vals"] > 0:
                        model.module.AGCT.update_weights_no_grad(agct_lr)
                ...
                optimizer.zero_grad()  # AZ ??
                total_loss = 0.0
                curr_acc_step = 1
                if (i + 1) % cfg["training"]["val_interval"] == 0:  # Evaluate the model
                    # evaluate_model()
