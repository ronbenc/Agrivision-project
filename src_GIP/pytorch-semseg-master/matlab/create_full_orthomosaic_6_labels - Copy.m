...
OUT_DIR = [BASE_DIR,CASE_DIR,'full_field\'];
...
for ii = 1:num_fields
    ...
    for jj = 1:num_images
        ...
        if ~strcmp(CASE_DIR,'test\')
            ...
            for kk = 2:num_labels
                label_file_name = [LABEL_DIRS{kk,1},curr_prefix,'.png'];
                curr_label = imread(label_file_name);
                idx = find(curr_label == 255);
                gt_label(idx) = kk-1;
            end        
        
            %idx = find(gt_label == 0);    %%% no category label
            %gt_label(idx) = num_labels;
