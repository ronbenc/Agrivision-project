...
from agrivision_utils import remove_small_components, blackout_image_border, save_confusion_matrix, multi_rotation_prediction
...
def validate(cfg, args):
    ...
    do_resize = False
    if cfg["data"]["img_rows"] != 512 or cfg["data"]["img_cols"] != 512:
        # in case model was trained on resized images, but we still want the validation on the original 512x512 images.
        do_resize = True
    ...
    for i, (images, labels) in enumerate(valloader):
        ...
        if do_resize is True:
            # pred[0] = cv2.resize(pred[0], dsize=(512,512), interpolation=cv2.INTER_NEAREST)
            # gt = cv2.resize(gt, dsize=(512, 512), interpolation=cv2.INTER_NEAREST)
            tmp = pred[0].copy()
            tmp = cv2.resize(tmp, dsize=(512, 512), interpolation=cv2.INTER_NEAREST)
            pred = np.zeros_like(pred,shape=(1,512,512))
            pred[0,:,:] = tmp

            gt = np.zeros_like(gt, shape=(1, 512, 512))
            gt[0,:,:] = loader.get_label_full_size(index=i)
            # tmp = gt[0].copy()
            # tmp = cv2.resize(tmp, dsize=(512, 512), interpolation=cv2.INTER_NEAREST)
            # gt = np.zeros_like(gt, shape=(1, 512, 512))
            # gt[0, :, :] = tmp
        ...
        if cfg["validation"]["out_pics"] > 0:
            if do_resize is True:
                # pred = cv2.resize(pred, dsize=(cfg["data"]["img_rows"], cfg["data"]["img_cols"]), interpolation=cv2.INTER_NEAREST)
                # gt = cv2.resize(gt, dsize=(cfg["data"]["img_rows"], cfg["data"]["img_cols"]), interpolation=cv2.INTER_NEAREST)
                tmp = pred[0].copy()
                tmp = cv2.resize(tmp, dsize=(cfg["data"]["img_rows"], cfg["data"]["img_cols"]), interpolation=cv2.INTER_NEAREST)
                pred = np.zeros_like(pred, shape=(1, cfg["data"]["img_rows"], cfg["data"]["img_cols"]))
                pred[0, :, :] = tmp
                gt = labels.numpy()
                # tmp = gt[0].copy()
                # tmp = cv2.resize(tmp, dsize=(cfg["data"]["img_rows"], cfg["data"]["img_cols"]), interpolation=cv2.INTER_NEAREST)
                # gt = np.zeros_like(gt, shape=(1, cfg["data"]["img_rows"], cfg["data"]["img_cols"]))
                # gt[0, :, :] = tmp
