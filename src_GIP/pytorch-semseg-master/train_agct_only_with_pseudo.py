...
from agrivision_utils import save_train_loss_info, save_val_loss_info,save_best_score, save_alpha_vals, save_best_alpha_vals
from ptsemseg.loader.agrivision_loader_utils import read_agct_coeffs

from ptsemseg.loader.agrivision6_loader_pseudo import agrivision6Loader_agct_pseudo
...
def train(cfg):
    ...
    t_loader_pseudo = agrivision6Loader_agct_pseudo(
        data_path,
        split="test",
        img_list="configs/agri2020_test_all.txt",
        img_size=(cfg["data"]["img_rows"], cfg["data"]["img_cols"]),
        n_channels=n_channels,
        augmentations=augmentations,
        debug_info=cfg["debug_info"],
    ...
    trainloader_pseudo = data.DataLoader(
        t_loader_pseudo,
        batch_size=cfg["training"]["batch_size"] // 2,
        num_workers=cfg["training"]["n_workers"],
        shuffle=True,
    ...
    model = get_model(cfg["model"], n_classes, n_channels, cfg["agct"], correct_block).to(device)
    ...
    val_loss_info = []
    if "agct" in cfg.keys():
        alpha_vals = np.zeros(shape=(cfg["agct"]["n_channels"],10),dtype=np.float)
        alpha_grad_vals = np.zeros(shape=(cfg["agct"]["n_channels"],10),dtype=np.float)
        if "alpha_trained" in cfg["agct"].keys() and os.path.isfile(cfg["agct"]["alpha_trained"]):
            alpha_vals = read_agct_coeffs(cfg["agct"]["alpha_trained"])
        else:
            alpha_vals = np.array(cfg["agct"]["alpha_coeffs"])
            alpha_vals = alpha_vals[0:cfg["agct"]["n_channels"],:]
    ...
    while i <= cfg["training"]["train_iters"] and flag:
        ...
        for (item1, item2) in zip(trainloader, trainloader_pseudo):

            image_batch1, label_batch1 = item1
            image_batch2, label_batch2 = item2
            images = torch.cat([image_batch1, image_batch2], dim=0)
            labels = torch.cat([label_batch1, label_batch2], dim=0)
            ...
            if "agct" in cfg.keys(): # cfg["agct"] is not  None:
                if cfg["agct"]["dynamic_vals"] > 0:
                    lr = cfg["agct"]["lr"]
                    model.module.AGCT.update_weights_no_grad(lr)
                if cfg["agct"]["print_vals"] > 0:
                    alpha, alpha_g = model.module.AGCT.get_weights()
                    alpha_vals = np.concatenate((alpha_vals,alpha), axis=0)
                    alpha_grad_vals = np.concatenate((alpha_grad_vals,alpha_g), axis=0)
                if cfg["agct"]["dynamic_vals"] > 0:
                    model.module.AGCT.update_weights_zero_grad()

                # alpha, alpha_g = model.module.AGCT.get_weights()
                # alpha_vals = np.concatenate((alpha_vals, alpha), axis=0)
                # alpha_grad_vals = np.concatenate((alpha_grad_vals, alpha_g), axis=0)
                # model.module.AGCT.update_weights(lr)
            ...
            if (i + 1) % cfg["training"]["val_interval"] == 0 or (i + 1) == cfg["training"]["train_iters"]:
                ...
                if "agct" in cfg.keys() and cfg["agct"]["print_vals"] > 0: # cfg["agct"] is not  None:
                    save_alpha_vals(logdir, alpha_vals, alpha_grad_vals, cfg["agct"]["n_channels"])
                ...
                if curr_iou >= best_iou:
                    ...
                    if "agct" in cfg.keys():
                        best_alpha, _ = model.module.AGCT.get_weights()
                        save_best_alpha_vals(logdir, best_alpha)
