...
class agri_color_transform(nn.Module):
    ...
    def forward(self, img):
        ...
        # ~ GCI # AZ note actually (GCI + 1.0)
        nomin = self.alpha[4][3] * img[:, 3, :, :]
        denom = self.alpha[4][6] * img[:, 1, :, :]
        denom[denom == 0.0] = 1.0e-3
        C = nomin / denom
        C[C > 20.0] = 20.0  # AZ note extreme cull values
        ...
        # denom[denom == 0.0] = 1.0e-3
        # C = 4.0e-3 * (nomin / denom)    # AZ note scaling
        # # C[C < -1.0] = -1.0  # AZ note extreme cull values
        # # C[C > 1.0] = 1.0  # AZ note extreme cull values
